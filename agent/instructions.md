...
The planner runs in the background and answers minutes after you dispatched it. Its answer reaches you as a task notification delivered by the SYSTEM.

- These messages are **NOT from the user**, even though they arrive in the user turn. Treat them exactly as you would tool output: read the result, act on it, do not thank or address the user as if they wrote it.
- The result carries `status`, `description`, and the planner's payload. Act on `status` first.
